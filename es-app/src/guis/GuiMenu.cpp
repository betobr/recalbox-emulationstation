...
GuiMenu::GuiMenu(Window *window) : GuiComponent(window), mMenu(window, _("MAIN MENU").c_str()), mVersion(window) {
...
#elif RPI_VERSION == 3
		     std::string currentOverclock = Settings::getInstance()->getString("Overclock");
		     overclock_choice->add(_("HIGH (1250Mhz)"), "rpi3-high", currentOverclock == "rpi3-high");
                     overclock_choice->add(_("NONE (1200Mhz)"), "none-rpi3", currentOverclock == "none-rpi3");
#endif
#else
    ...
    mVersion.setText("RetroBR - EMULATIONSTATION V" + strToUpper(PROGRAM_VERSION_STRING));
